...
function handleObject(object, recurFunc, remLen, fullLen) {
    var keys = Object.keys(object),
        ...
        return { key: k, val: recurFunc(object[k], remLen - k.length, fullLen) };
    }),
        ...
        joined = resSameLine.map(function (e) {
        ...
        return e.length > fullLen;
    }),
        resNextLine = keys.map(function (k) {
        return { key: k, val: recurFunc(object[k], remLen - 2, fullLen) };
    }),
...
function fromJSONObjectRecursive(object, remLen, fullLen) {
    ...
    switch (typeof object === 'undefined' ? 'undefined' : _typeof(object)) {
        ...
        case "object":
            var res = Array.isArray(object) ? handleArray(object, fromJSONObjectRecursive, remLen) : handleObject(object, fromJSONObjectRecursive, remLen, fullLen);

            return res;
    }
}

function fromJSONObject(object, lineLength) {
...
function highlightION(text, keyStyleString, keywordStyleString) {
    return text.split('\n').map(function (t) {
        return t.replace(/(\S+\s*)(?=:)/g, '<b class="' + keyStyleString + '">$1</b>').replace(/([#@:])/g, '<b class="' + keywordStyleString + '">$1</b>');
    }).join('\n');
}
